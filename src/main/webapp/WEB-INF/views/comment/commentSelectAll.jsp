...
<fieldset
	style="margin: auto; position: relative; top: 200px; width: 1200px; border: 1px solid transparent;">
	...
	<h2>查詢留言紀錄</h2>
		...
		<table class="selectrecord">
			...
			<FORM ACTION="${pageContext.request.contextPath}/comment/CommentController" method="Post">
					...
					<td class="selectrecord"><%=list.get(i).getMember().getName()%></td>
					<td class="selectrecord"><%=list.get(i).getMember().getGender()%></td>
					<td class="selectrecord"><%=list.get(i).getMember().getBirth()%></td>
					...
					<td class="selectrecord">
					<button style="width: auto; height: auto; margin-top: 0px;" 
					name="update" value=<%=list.get(i).getCommentId()%>>更新</button></td>
					<td class="selectrecord">
					<button style="width: auto; height: auto; margin-top: 0px;" 
					name="delete" value=<%=list.get(i).getCommentId()%>>刪除</button></td>
					</FORM>
			</tr>
